...
6. Успешно найденный корректный SQL-ответ сохраняется в `AssessmentAnswer` и не затирается последующим неудачным запуском того же вопроса.
...
- sample data показываются прямо внутри карточек всех таблиц на диаграмме, чтобы стажёр видел и структуру, и данные одновременно
- сид добавляет несколько SQL-пресетов для быстрой ручной проверки на чистой базе, включая задачи про `SQL`-навык, `Математику`, покупки клиента `ООО Стройка` и сотрудников отдела `ИТ`
...
- в итоговом сохранении для SQL учитывается лучший найденный корректный запуск, а не последний неудачный прогон
